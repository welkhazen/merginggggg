            ...
            return (
              ...
              >
                ...
                <div className="relative flex items-center justify-between px-4 py-2.5">
                  ...
                  {hasVoted && <span className="text-[11px] font-medium text-raw-gold/60">{pct}%</span>}
                </div>
              </button>
            );
          })}
        </div>
        {hasVoted && <p className="mt-3 text-[10px] text-raw-silver/25">{totalVotes.toLocaleString()} anonymous responses</p>}
      </div>
    </GlareCard>
  );
}
...
export function DashboardPolls({
  ...
  const totalResponses = polls.reduce((sum, p) => sum + p.options.reduce((s, o) => s + o.votes, 0), 0);

  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl tracking-wide text-raw-text">Polls</h1>
        <p className="mt-2 text-sm text-raw-silver/40">Answer anonymously. See live results. Shape the community signal.</p>
      </div>
      ...
      <RawInsightsPanel
        ...
        onPurchaseInsight={onPurchaseInsight}
      />
