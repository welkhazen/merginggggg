...
export function UsersTab() {
  ...
  const [status, setStatus] = useState<"all" | "active" | "warned" | "banned">("all");
  const [search, setSearch] = useState({ q: "", status: "all" });
  const { data: users, loading, reload } = useAsyncData(
    () => fetchUsers({ q: search.q || undefined, status: search.status, limit: 50 }),
    [search],
  );
  ...
  return (
    <>
      <Panel
        ...
        actions={
          <AdminButton tone="outline" disabled={loading} onClick={reload} aria-label="Refresh users">
      ...
      >
        ...
        >
          <Field value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search username" />
        ...
        {users && users.length === 0 && <EmptyState>No users match.</EmptyState>}
            ...
            <Row key={user.id}>
              <div className="min-w-0">
                <p className="flex flex-wrap items-center gap-2 text-sm font-semibold text-raw-text">
                  ...
                  {user.warnings > 0 && <Tag tone="red">{user.warnings} warnings</Tag>}
                </p>
      ...
      {detail && <UserDetailPanel detail={detail} onRefresh={() => void openDetail(detail.user)} onClose={() => setDetail(null)} />}
      <AppealsPanel />
    </>
  );
}
...
function UserDetailPanel({ detail, onRefresh, onClose }: { detail: UserDetail; onRefresh: () => void; onClose: () => void }) {
  ...
  async function run(action: ModerationAction, minutes?: number) {
    ...
    try {
      ...
      captureAdminEvent("admin_user_moderated", { action, minutes, target_username: user.username });
      toast({ title: "Action applied", description: `@${user.username} updated.` });
      onRefresh();
    } catch (error) {
      captureAdminException(error, { action: "admin_user_moderation", moderation_action: action });
      toast({ title: "Action failed", description: error instanceof Error ? error.message : undefined });
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <Panel
      ...
      actions={<AdminButton tone="outline" onClick={onClose}>Close</AdminButton>}
    >
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-2 text-xs text-raw-silver/55">
          ...
          {user.moderationStatus && user.moderationStatus !== user.status && (
            <Tag tone={statusTone(user.moderationStatus)}>{user.moderationStatus}</Tag>
          )}
          {user.tier && <Tag tone="gold">{TIER_LABELS[user.tier]}</Tag>}
          <span>{user.warnings} warnings</span>
          <span>{user.spamStrikes} spam strikes</span>
          {user.bannedUntil && <span>Banned until {formatDate(user.bannedUntil)}</span>}
          {detail.safetyScore && (
            <span>
              Safety {detail.safetyScore.score} ({detail.safetyScore.total_flags} flags, {detail.safetyScore.total_reports_against} reports)
            </span>
          )}
        </div>

        <div className="flex flex-wrap gap-2">
          <AdminButton tone="outline" disabled={pending} onClick={() => void run("warn")}>
            <TriangleAlert className="h-4 w-4" /> Warn
          </AdminButton>
          {TIMEOUTS.map((timeout) => (
            <AdminButton key={timeout.minutes} tone="teal" disabled={pending} onClick={() => void run("timeout", timeout.minutes)}>
              Timeout {timeout.label}
            </AdminButton>
          ))}
          <AdminButton tone="danger" disabled={pending} onClick={() => void run("ban")}>
            <Ban className="h-4 w-4" /> Ban
          </AdminButton>
          <AdminButton tone="teal" disabled={pending} onClick={() => void run("unban")}>
            <ShieldCheck className="h-4 w-4" /> Unban
          </AdminButton>
        </div>

        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-raw-silver/50">Moderation history</p>
          {detail.actions.length === 0 && <EmptyState>No actions recorded.</EmptyState>}
          <div className="space-y-1">
            {detail.actions.map((action) => (
              <p key={action.id} className="text-xs text-raw-silver/60">
                <span className="font-semibold text-raw-text">{action.action}</span> · {formatDate(action.createdAt)}
                {action.reason && <span> — {action.reason}</span>}
                {action.expiresAt && <span> (until {formatDate(action.expiresAt)})</span>}
              </p>
            ))}
          </div>
        </div>
        ...
        {detail.appeals.length > 0 && (
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-raw-silver/50">Appeals</p>
            <div className="space-y-1">
              {detail.appeals.map((appeal) => (
                <p key={appeal.id} className="text-xs text-raw-silver/60">
                  <Tag tone={statusTone(appeal.status)}>{appeal.status}</Tag> {formatDate(appeal.createdAt)} — {appeal.text}
                </p>
              ))}
            </div>
          </div>
...
function AppealsPanel() {
  ...
  const { data: appeals, loading, reload } = useAsyncData(() => fetchAppeals(status), [status]);
    ...
    <Panel
      ...
      actions={
        <div className="flex gap-2">
          ...
          <AdminButton tone="outline" disabled={loading} onClick={reload} aria-label="Refresh appeals">
            <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
          </AdminButton>
        </div>
      }
    >
      {appeals && appeals.length === 0 && <EmptyState>No appeals in this view.</EmptyState>}
      <div className="space-y-2">
        {appeals?.map((appeal) => (
          <Row key={appeal.id}>
            <div className="min-w-0 flex-1">
              ...
              {appeal.reviewedBy && (
                <p className="mt-1 text-xs text-raw-silver/40">Reviewed by {appeal.reviewedBy} {formatDate(appeal.reviewedAt)}</p>
              )}
            </div>
            {appeal.status === "pending" && (
              <div className="flex gap-2">
                <AdminButton tone="teal" onClick={() => void review(appeal.id, "approved")}>Approve</AdminButton>
                <AdminButton tone="danger" onClick={() => void review(appeal.id, "denied")}>Deny</AdminButton>
              </div>
            )}
          </Row>
        ))}
