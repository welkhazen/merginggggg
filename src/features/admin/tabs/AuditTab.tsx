...
import { AdminButton, EmptyState, Field, Panel, Row, Tag } from "../ui";
          ...
          <Row key={entry.id}>
            <div className="min-w-0 flex-1">
              <p className="flex flex-wrap items-center gap-2 text-sm text-raw-text">
                ...
                <span className="font-mono text-xs text-raw-silver/70">
                  {entry.action}
                </span>
                {entry.targetLabel && (
                  <span className="text-raw-silver/60">
                    → {entry.targetLabel}
                  </span>
                )}
              </p>
              {Object.keys(entry.details ?? {}).length > 0 && (
                <p className="mt-1 break-all font-mono text-[10px] text-raw-silver/40">
