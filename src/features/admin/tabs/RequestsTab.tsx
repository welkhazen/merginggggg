...
export function RequestsTab({ currentTier }: { currentTier: StaffTier }) {
  ...
  const {
    ...
    loading,
    reload,
  } = useAsyncData(() => fetchCommunityRequests(status), [status]);
  ...
  async function review(id: string, outcome: "approved" | "rejected") {
    try {
      ...
      captureAdminEvent("admin_community_request_reviewed", {
        status: outcome,
      });
      ...
      captureAdminException(error, {
        action: "admin_community_request_review",
      });
        ...
        {requests?.map((request) => (
          <Row key={request.id}>
            <div className="min-w-0 flex-1">
              ...
              {request.audience && (
                <p className="text-xs text-raw-silver/60">
