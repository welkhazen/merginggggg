...
export function ReportsTab() {
  ...
  const { data: reports, loading, reload } = useAsyncData(() => fetchReports(status), [status]);
  ...
  async function review(id: string, outcome: "resolved" | "dismissed") {
    try {
      ...
      toast({ title: outcome === "resolved" ? "Report resolved" : "Report dismissed" });
      reload();
    } catch (error) {
    ...
    <Panel
      ...
      actions={
        <div className="flex gap-2">
          ...
          <AdminButton tone="outline" disabled={loading} onClick={reload} aria-label="Refresh reports">
            <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
          </AdminButton>
        </div>
      }
    >
      {reports && reports.length === 0 && <EmptyState>No reports here. Nice and quiet.</EmptyState>}
          ...
          <Row key={report.id}>
            <div className="min-w-0 flex-1">
              <p className="flex flex-wrap items-center gap-2 text-xs text-raw-silver/55">
                ...
                {report.communityTitle && <span>in {report.communityTitle}</span>}
              </p>
              <p className="mt-1 text-sm text-raw-text">
                <span className="font-semibold">@{report.reporterName ?? "unknown"}</span> reported{" "}
                <span className="font-semibold">@{report.reportedUsername ?? "unknown"}</span>
                {report.reason && <span className="text-raw-silver/60"> — {report.reason}</span>}
              </p>
              {report.messageText && (
              ...
              {report.details && <p className="mt-1 text-xs text-raw-silver/45">{report.details}</p>}
              {report.resolvedBy && (
            ...
            {report.status === "open" && (
              <div className="flex gap-2">
                <AdminButton tone="teal" onClick={() => void review(report.id, "resolved")}>Resolve</AdminButton>
                <AdminButton tone="outline" onClick={() => void review(report.id, "dismissed")}>Dismiss</AdminButton>
              </div>
            )}
          </Row>
        ))}
