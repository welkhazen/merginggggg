  ...
  return (
    <>
      <Panel
        ...
        actions={
          <AdminButton tone="outline" disabled={loading} onClick={reload} aria-label="Refresh staff">
            <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
          </AdminButton>
        }
      >
        {staff && staff.length === 0 && <EmptyState>No staff accounts.</EmptyState>}
        <div className="space-y-2">
          {staff?.map((member) => (
            <StaffRow key={member.id} member={member} currentUser={currentUser} onChanged={reload} />
          ))}
        </div>
      </Panel>
...
function StaffRow({ member, currentUser, onChanged }: { member: StaffMember; currentUser: AdminUser; onChanged: () => void }) {
  ...
  const assignableTiers = STAFF_TIERS.filter((tier) => {
    ...
    if (tierAtLeast(tier, "owner") && currentUser.tier !== "super_admin") return false;
  ...
  async function setTier(tier: StaffTier | null) {
    ...
    try {
      ...
      toast({ title: tier ? `@${member.username} is now ${TIER_LABELS[tier]}` : `@${member.username} removed from staff` });
      onChanged();
    } catch (error) {
      captureAdminException(error, { action: "admin_staff_tier_change" });
      toast({ title: "Could not change tier", description: error instanceof Error ? error.message : undefined });
    } finally {
      setPending(false);
    }
  }
...
function CreateStaffPanel({ currentUser, onCreated }: { currentUser: AdminUser; onCreated: () => void }) {
  ...
  const creatableTiers = STAFF_TIERS.filter((candidate) => {
    ...
    if (tierAtLeast(candidate, "owner") && currentUser.tier !== "super_admin") return false;
    return true;
  });
  ...
  async function submit() {
    ...
    try {
      ...
      captureAdminException(error, { action: "admin_staff_account_create", tier });
