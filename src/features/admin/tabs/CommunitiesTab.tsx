...
export function CommunitiesTab() {
  ...
  async function toggleLock(community: CommunitySummary) {
    ...
    try {
      ...
      captureAdminEvent("admin_community_lock_toggled", {
  ...
  function requestDelete(community: CommunitySummary) {
    toast({
      ...
      description:
        "This permanently removes the room and its messages. This cannot be undone.",
      action: (
        <ToastAction
          altText={`Delete ${community.title}`}
          onClick={() => void confirmDelete(community)}
        >
          Delete
        </ToastAction>
      ),
  ...
  async function confirmDelete(community: CommunitySummary) {
    ...
    try {
      ...
      captureAdminEvent("admin_community_deleted", {
        community_id: community.id,
      });
          ...
          {[
            {
              ...
              title: "Unlocked rooms",
              tone: "green" as const,
              rooms: groupedCommunities.unlocked,
            },
            {
              key: "locked" as const,
              title: "Locked rooms",
              tone: "red" as const,
              rooms: groupedCommunities.locked,
            },
          ].map((section) => {
            ...
            return (
              <section
                key={section.key}
                className="rounded-2xl border border-raw-border/20 bg-raw-black/20"
              >
                <button
                  type="button"
                  onClick={() =>
                ...
                >
                  <span className="flex min-w-0 items-center gap-2">
                    ...
                    <span className="text-xs text-raw-silver/45">
                      {section.rooms.length} room
                ...
                {isOpen && (
                  <div className="space-y-2 border-t border-raw-border/15 p-2">
                    {section.rooms.length === 0 && (
                      <p className="px-2 py-4 text-sm text-raw-silver/40">
                        No {section.key} rooms.
                      </p>
                    )}
                      ...
                      return (
                        <Row key={community.id}>
                          ...
                          <div className="flex gap-2">
                            ...
                            <AdminButton
                              tone={community.locked ? "teal" : "danger"}
                              disabled={savingLock}
                              onClick={() => void toggleLock(community)}
                            >
                              {community.locked ? (
...
function CommunityInspector({ community }: { community: CommunitySummary }) {
  ...
  const {
    ...
    () => fetchCommunityMessages(community.id, filter),
    [community.id, filter],
  );
  const [realtimeFallback, setRealtimeFallback] = useState(
    !isCommunityRealtimeConfigured(),
  );
  ...
  const filteredMessages = useMemo(() => {
    ...
    const messages = [...(messageData ?? [])].sort(
      (a, b) =>
    ...
    const channel = subscribeToCommunityMessages(
      ...
      (message, eventType) => {
        ...
        setMessageData((current) => {
          ...
          if (eventType === "DELETE")
            return messages.filter((item) => item.id !== message.id);
  ...
  async function removeMessage(message: CommunityMessage) {
    const reason = window.prompt(
      `Delete this message from @${message.senderName ?? "unknown"}?\nOptional reason:`,
    );
    if (reason === null) return; // cancelled
    try {
      await deleteCommunityMessage(message.id, reason || undefined);
      captureAdminEvent("admin_message_deleted", {
        community_id: community.id,
      });
  ...
  async function moderateSender(
    ...
    try {
      ...
      toast({
        title:
          action === "ban" ? `@${username} banned` : `@${username} timed out`,
        description:
          action === "timeout" && minutes ? `${minutes} minutes` : undefined,
      });
      reloadMessages();
    } catch (error) {
      captureAdminException(error, { action: "admin_message_moderate_sender" });
      toast({
        title: "Could not moderate user",
        description: error instanceof Error ? error.message : undefined,
      });
    }
  }
  ...
  async function sendMessage(event: React.FormEvent) {
    ...
    try {
      ...
      captureAdminEvent("admin_community_message_sent", {
        community_id: community.id,
  ...
  return (
    <Panel
      ...
      hint={
        realtimeFallback
          ? "Realtime is unavailable, so this view is temporarily falling back to 3-second refreshes."
          : "Realtime is active for messages; deletions here are soft deletes visible to the main app."
      }
      actions={
        <div className="flex gap-2">
          <SelectField
          ...
          <div className="mb-4 rounded-[18px] border border-[#ded8cb] bg-[#f8f5ee] px-5 py-4 text-[#1f2528] shadow-[0_8px_28px_rgba(0,0,0,0.08)]">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="truncate font-display text-xl leading-tight tracking-wide text-[#171717]">
                    {community.title}
                  </p>
                  <p className="mt-1 text-xs text-[#77736b]">
                    {community.topic ?? "Live community room"}
                  </p>
                </div>
              </div>
          ...
          <div className="flex h-[min(760px,calc(100dvh-180px))] min-h-[620px] flex-col overflow-hidden rounded-[18px] border border-[#ded8cb] bg-[#fbfaf7] text-[#1f2528] shadow-[0_10px_36px_rgba(0,0,0,0.1)]">
            <div className="flex flex-wrap items-center gap-2 border-b border-[#e6e0d5] px-4 py-3">
              <div className="flex min-w-[220px] flex-1 items-center gap-2 rounded-xl border border-[#d9d2c7] bg-white px-3 py-2">
                ...
                {searchQuery && (
                  <button
                    ...
                    className="rounded-full p-0.5 text-[#9b968d] hover:text-[#1f2528]"
              ...
              <SelectField
                ...
                className="border-[#d9d2c7] bg-white text-[#1f2528]"
              >
                <option value="all">All</option>
                <option value="deleted">Deleted</option>
                <option value="flagged">Flagged</option>
            ...
            >
                ...
                <div key={group.label} className="space-y-3">
                  ...
                  {group.messages.map((message) => {
                    const isModeratorMessage =
                      (message.senderName ?? "")
                        .toLowerCase()
                        .includes("moderator") || message.senderId === "admin";
                    return (
                      <article
                        key={message.id}
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex min-w-0 flex-wrap items-center gap-2">
                            <span
                              className={
                                isModeratorMessage
                                  ? "text-lime-700 text-xs font-semibold"
                            ...
                            <span
                              className={`min-w-0 break-words text-xs ${message.isDeleted ? "italic text-[#9b968d] line-through" : "text-[#3b3a36]"}`}
                            >
                              {message.text}
                            </span>
                            <span className="shrink-0 text-[10px] text-[#9b968d]">
                              {formatChatTime(message.createdAt)}
                            </span>
                            {message.isDeleted && (
                              <span className="rounded border border-red-200 bg-red-50 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-red-500">
                                Deleted
                              </span>
                            )}
                            {message.moderationStatus &&
                              message.moderationStatus !== "removed" && (
                                <span className="rounded border border-red-300 bg-red-50 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-red-600">
                                  {message.moderationStatus}
                                </span>
                              )}
                          </div>
                          ...
                          <div className="ml-auto flex shrink-0 gap-1 opacity-100 transition-opacity sm:opacity-0 sm:group-hover/msg:opacity-100">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    void moderateSender(message, "timeout", 10)
                                  }
                                >
                                  <Clock className="mr-2 h-3.5 w-3.5" /> Timeout
                                  10 min
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() =>
                                    void moderateSender(message, "timeout", 60)
                                  }
                                >
                                  <Clock className="mr-2 h-3.5 w-3.5" /> Timeout
                                  1 hour
                                ...
                                <DropdownMenuItem
                                  ...
                                  onClick={() =>
                                    void moderateSender(message, "ban")
                                  }
                                >
              ...
              {messageData && messageData.length === 0 && (
                <EmptyState>No messages match this filter.</EmptyState>
              )}
              {messageData &&
                messageData.length > 0 &&
                groupedMessages.length === 0 && (
                  <EmptyState>No messages match your search.</EmptyState>
                )}
            </div>

            <form
              onSubmit={sendMessage}
              className="border-t border-[#e6e0d5] bg-[#fbfaf7] px-3 py-3"
            >
              {replyTo && (
                <div className="mb-2 flex items-start justify-between gap-3 rounded-xl border border-lime-300 bg-lime-50 px-3 py-2 text-xs text-[#6f6a62]">
                  ...
                  <button
                    type="button"
                    className="text-[#8f887e] hover:text-[#1f2528]"
        ...
        <div className="space-y-2">
          ...
          {members.data?.map((member) => (
            <Row key={member.userId}>
              <div>
                <p className="text-sm font-semibold text-raw-text">
                  @{member.username ?? member.userId}
                </p>
                <p className="text-xs text-raw-silver/45">
                  Joined {formatDate(member.joinedAt)} · Last seen{" "}
