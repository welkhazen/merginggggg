...
export function FlagsTab() {
  ...
  const {
    ...
    loading,
    reload,
  } = useAsyncData(() => fetchFlags(reviewed), [reviewed]);
  ...
  return (
      ...
      actions={
        <div className="flex gap-2">
          <SelectField
            value={reviewed}
            onChange={(event) =>
              setReviewed(event.target.value as typeof reviewed)
            }
          >
    ...
    >
      {flags && flags.length === 0 && (
        <EmptyState>No flags in this view.</EmptyState>
      )}
          ...
          <Row key={flag.id}>
            ...
            {!flag.reviewed && (
              <div className="flex gap-2">
                <AdminButton
                  tone="danger"
                  onClick={() => void review(flag.id, "violation")}
                >
                  Violation
                </AdminButton>
                <AdminButton
                  tone="teal"
                  onClick={() => void review(flag.id, "ok")}
                >
                  OK
                </AdminButton>
                <AdminButton
                  tone="outline"
                  onClick={() => void review(flag.id, "unclear")}
                >
                  Unclear
                </AdminButton>
              </div>
