...
function WaitlistPanel() {
  const [status, setStatus] = useState<WaitlistRequestStatus | "all">(
    "pending",
  );
  const {
    data: requests,
    loading,
    reload,
  } = useAsyncData(() => fetchWaitlistRequests(status), [status]);
  ...
  async function setRequestStatus(id: string, next: WaitlistRequestStatus) {
    ...
    try {
      ...
      toast({
        title: `Request marked ${WAITLIST_STATUS_LABELS[next].toLowerCase()}`,
      });
      reload();
    } catch (error) {
  ...
  return (
    ...
    >
      {requests && requests.length === 0 && (
        <EmptyState>No waitlist requests in this view.</EmptyState>
      )}
              ...
              <div className="min-w-0 flex-1">
                <p className="flex flex-wrap items-center gap-2 text-sm font-semibold text-raw-text">
                  ...
                  <Tag tone={statusTone(request.status)}>
                    {WAITLIST_STATUS_LABELS[request.status] ?? request.status}
                  </Tag>
                </p>
                <p className="mt-0.5 text-xs text-raw-silver/50">
                  via {request.source.replaceAll("_", " ")} ·{" "}
              ...
              <div className="flex flex-wrap gap-2">
                {request.status === "pending" && (
                  <AdminButton
                    ...
                    disabled={busy}
                    onClick={() =>
                      void setRequestStatus(request.id, "contacted")
                    }
                  >
                    Mark contacted
                  </AdminButton>
                ...
                {(request.status === "pending" ||
                  ...
                  <AdminButton
                    disabled={busy}
                    onClick={() =>
                      void setRequestStatus(request.id, "sent_code")
                    }
                  >
                ...
                {request.status !== "closed" && (
                  <AdminButton
                    tone="outline"
                    disabled={busy}
                    onClick={() => void setRequestStatus(request.id, "closed")}
                  >
                    Close
                  </AdminButton>
                )}
                {request.status === "closed" && (
                  <AdminButton
                    tone="outline"
                    disabled={busy}
                    onClick={() => void setRequestStatus(request.id, "pending")}
                  >
                    Reopen
                  </AdminButton>
                )}
...
function GrantInvitesPanel({ currentUsername }: { currentUsername: string }) {
  ...
  async function grant(target: string, amount: number) {
    ...
    try {
      ...
      captureAdminEvent("admin_invite_codes_granted", {
        amount: codes.length,
        target_self: target === currentUsername,
      });
      toast({
        title: "Invite codes granted",
        description: `${codes.length} code(s) created for @${target}.`,
      });
      if (target !== currentUsername) setUsername("");
    } catch (error) {
      captureAdminException(error, {
        action: "admin_invite_codes_grant",
        amount,
      });
      toast({
        title: "Could not grant codes",
        description:
  ...
  return (
    <Panel
      title="Grant invite codes"
      hint="Give a user extra founding invite codes beyond their base allotment."
    >
      <div className="space-y-3">
        <div className="grid gap-2 sm:grid-cols-[1fr_90px_auto] sm:items-center">
          <div>
            <p className="text-sm font-medium text-raw-text">
              Generate for yourself
            </p>
            <p className="text-xs text-raw-silver/40">
              Create a batch of invite codes on @{currentUsername} to send out.
            </p>
          </div>
          <Field
            type="number"
          ...
          <AdminButton
            disabled={loading}
            onClick={() =>
              void grant(currentUsername, Number.parseInt(selfCount, 10))
            }
          >
        ...
        <div className="grid gap-2 sm:grid-cols-[1fr_90px_auto]">
          ...
          <AdminButton
            ...
            onClick={() =>
              void grant(username.trim(), Number.parseInt(count, 10))
            }
          >
...
function RedemptionsPanel() {
  const {
    ...
    loading,
    reload,
  } = useAsyncData(fetchInviteRedemptions);
          ...
          <Row key={redemption.id}>
            <div>
              <p className="text-sm text-raw-text">
                ...
                {redemption.redeemedUsername && (
                  <span> → @{redemption.redeemedUsername}</span>
                )}
              </p>
